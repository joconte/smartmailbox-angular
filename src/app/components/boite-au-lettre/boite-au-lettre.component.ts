...
export class BoiteAuLettreComponent implements OnInit {
  ...
  async postNewMail(boiteAuLettre: BoiteAuLettre) {
    let loaded: boolean = await this.courrierService.postCourrier(boiteAuLettre);
    let boiteAuLettreInDb: BoiteAuLettre = await this.boiteAuLettreService.getBoiteAuLettreById(boiteAuLettre.id) as BoiteAuLettre;
